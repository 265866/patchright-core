...
/* BUN_WS_PATCHED */
// Bun-native WebSocket adapter with ws-compatible EventEmitter API.
// Bun's global WebSocket doesn't accept the options object that the 'ws'
// library expects (maxPayload, agent, perMessageDeflate, etc.), so we
// wrap it in an adapter that exposes the same .on/.once/.off surface
// that the rest of transport.ts relies on.
class BunWsAdapter {
  ...
  readyState = 0;
  private _listeners: Record<string, Array<{ fn: (...args: any[]) => void; once: boolean }>> = {};
  private _rawWs: globalThis.WebSocket;

  constructor(url: string, _protocols?: any, _opts?: any) {
    const raw = new globalThis.WebSocket(url);
    raw.binaryType = 'arraybuffer';
    this._rawWs = raw;
    raw.onopen = () => { this.readyState = 1; this._emit('open'); };
    raw.onclose = (e: any) => { this.readyState = 3; this._emit('close', { code: e.code, reason: e.reason }); };
    raw.onerror = () => { this._emit('error', { message: 'WebSocket error', type: 'error' }); };
    raw.onmessage = (e: any) => { this._emit('message', { data: e.data }); };
  }
  ...
  private _emit(event: string, ...args: any[]) {
    ...
    for (let i = 0; i < listeners.length; i++) {
      ...
      entry.fn(...args);
      if (entry.once) { listeners.splice(i, 1); i--; }
    }
  }
  ...
  addEventListener(event: string, fn: (...args: any[]) => void) { return this.on(event, fn); }
  removeEventListener(event: string, fn: (...args: any[]) => void) { return this.off(event, fn); }
  send(data: any) { this._rawWs.send(data); }
  close(code?: number, reason?: string) { this._rawWs.close(code, reason); }
}
...
export class WebSocketTransport implements ConnectionTransport {
  ...
  constructor(progress: Progress|undefined, url: string, logUrl: string, options: WebSocketTransportOptions) {
    this.wsEndpoint = url;
    this._logUrl = logUrl;
    this._ws = new (_useBunWs ? BunWsAdapter as any : ws)(url, [], {
  ...
  async closeAndWait() {
    if (this._ws.readyState === (_useBunWs ? BunWsAdapter : ws).CLOSED)
      return;
